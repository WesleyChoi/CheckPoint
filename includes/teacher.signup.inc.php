...
if(isset($_POST['signup-submit'])) {
    ...
    if (empty($firstname) || empty($lastname) || empty($username) || empty($password) || empty($passwordrepeat)) {
        header("Location: ../teachersignup.php?error=emptyfields&uid=".$username);
        exit();
    }
    else if ($password !== $passwordrepeat) {
        header("Location: ../teachersignup.php?error=passwordcheck&uid=".$username);
        exit();
    }
    else {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            header("Location: ../teachersignup.php?error=sqlerror");
            exit();
        }
        else {
            ...
            if ($resultCheck > 0) {
                header("Location: ../teachersignup.php?error=usertaken");
                exit();
            }
            else{
